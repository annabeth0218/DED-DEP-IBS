...
df <- read_table('figs/eqtl_heatmap/genes_t2.txt')
genes <- df$gene
...
# Load data
eqtl_ded <- process_eqtl_data("fuma/fuma_ded/eqtl.txt", "red")  # Dry Eye (DED)
eqtl_dep <- process_eqtl_data("fuma/fuma_dep/eqtl.txt", "blue") # Depression (DEP)
...
combined_data$Category <- factor(combined_data$Category, levels = c("red", "blue", "#BA8E23"))
...
# Save plot
ggsave("heatmap_eqtl_modified.png", plot = p, width = 24, height = 14, dpi = 300)
